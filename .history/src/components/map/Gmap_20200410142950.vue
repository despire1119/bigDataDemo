<template>
  <div class="map-room" @dblclick="moveTo" @click="addmarker">
    <div style="height:100%;width:100%" id="container" tabindex="0"></div>
  </div>
</template>

<script>
import Qmap from './index'
export default {
  data() {
    return {
      qmap: {}
    }
  },
  mounted() {
    this.init()
    //零时模拟
    // setInterval(() => {
    //   let random = Math.random() * 0.005
    //   const that = this
    //   return (function(x) {
    //     console.log(106.546331 + x)
    //    that.qmap.addMarker( 106.546331 + x,  29.558136  + x)
    //    that.qmap.moveToPoint( 106.546331 + x,  29.558136  + x)
    //   })(random)
    // }, 3000)
  },
  methods: {
    init() {
      const bass = {
        id: container,
        x: 106.546331,
        y: 29.558136,
        zoom: 17,
        pitch: 65,
        mode: '3D'
      }
      this.qmap = new Qmap(bass)
      // this.qmap.on('moveend', logMapBounds)
      // this.qmap.on('zoomend', logMapBounds)
    },
    moveTo() {
      this.qmap.moveToPoint(116.319665, 39.855919)
    },
    addmarker() {
      this.qmap.addMarker(
        106.546331 + Math.random() * 0.005,
        29.558136 + Math.random() * 0.005
      )
    }
  }
}
</script>

<style lang="less" scoped>
.map-room {
  position: relative;
  width: 100%;
  height: 100%;
}
</style>
