<template>
  <div class="map-room">
    ...
    <!-- <video-cover/> -->
    <weather />
    <div id="container" style="height:100%;width:100%" tabindex="0" @click="mapClickHandle"  />
  </div>
</template>
...
export default {
  ...
  methods: {
    mapClickHandle(){
      this.qmap.removeInfoWindow(this.qmap.infoWindow)
    },
    bind() {
    ...
    pClick(p) {
      ...
      const ctx = `<video src="video/trafficModel.mp4" controls="controls" autoplay="autoplay" loop="loop" preload></video>`
      this.qmap.openInfo(ctx,lng,lat)
